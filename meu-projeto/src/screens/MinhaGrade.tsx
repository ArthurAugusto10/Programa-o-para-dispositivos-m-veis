...
export default function MinhaGrade({ route }: any) {
    ...
    useEffect(() => {
        // 🔬 LOG DETECTIVE: Olhe o terminal do seu VS Code / Metro Bundler ao abrir a tela!
        console.log("📧 E-mail que chegou na tela MinhaGrade:", emailUsuario);
        
        if (emailUsuario) {
            carregarDados();
        } else {
            console.warn("⚠️ Atenção: Nenhum e-mail foi recebido via rotas!");
            setLoading(false);
        }
    }, [emailUsuario]);
    ...
    const carregarDados = async () => {
        ...
        try {
            ...
            // 🔬 LOG DE RESPOSTA DO BANCO: Ver o que o backend respondeu
            console.log("📦 Dados das matérias retornados:", resMaterias.data);
